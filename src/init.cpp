...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: OuiCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: OuiCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeOuiCoinamount=<n> " + _("Keep N OuiCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. OuiCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. OuiCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("OuiCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "OuiCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of OuiCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart OuiCoin to complete") << "\n";
    ...
    nAnonymizeOuiCoinAmount = GetArg("-anonymizeOuiCoinamount", 0);
    if(nAnonymizeOuiCoinAmount > 999999) nAnonymizeOuiCoinAmount = 999999;
    if(nAnonymizeOuiCoinAmount < 2) nAnonymizeOuiCoinAmount = 2;
    ...
    LogPrintf("Anonymize OuiCoin Amount %d\n", nAnonymizeOuiCoinAmount);

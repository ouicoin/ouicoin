...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOuiCoinAmount"))
        settings.setValue("nAnonymizeOuiCoinAmount", 1000);
    nAnonymizeOuiCoinAmount = settings.value("nAnonymizeOuiCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeOuiCoinAmount"))
        SoftSetArg("-anonymizeOuiCoinamount", settings.value("nAnonymizeOuiCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeOuiCoinAmount:
            return QVariant(nAnonymizeOuiCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeOuiCoinAmount:
            nAnonymizeOuiCoinAmount = value.toInt();
            settings.setValue("nAnonymizeOuiCoinAmount", nAnonymizeOuiCoinAmount);
            emit AnonymizeOuiCoinAmountChanged(nAnonymizeOuiCoinAmount);
            break;
        default:
            break;
        }

...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    COuiCoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, QString txcomment, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(COuiCoinAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        int nChangePos;
        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strTxComment, nChangePos, strFailReason, coinControl, recipients[0].inputType, recipients[0].useInstantX);
        ...
        // reject ouicoin fee
        unsigned int ouicoinfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        ouicoinfee = ((MIN_RELAY_TX_FEE * ouicoinfee) * 10000);
        if (nFeeRequired > ouicoinfee){
            LogPrintf("nFeeRequired: %d -- InsaneFee: %d\n", nFeeRequired, ouicoinfee);
            return InsaneFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, QString txcomment, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(COuiCoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
        ...
        bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, strTxComment, nChangePos, strFailReason, coinControl, recipients[0].inputType, recipients[0].useInstantX);
        transaction.setTransactionFee(nFeeRequired);
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = COuiCoinAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(COuiCoinAddress(address).ToString());
...
{
    // OuiCoin context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(COuiCoinAddress(address).ToString())].push_back(out);
    }
}

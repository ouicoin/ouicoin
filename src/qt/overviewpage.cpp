...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeOuiCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeOuiCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeOuiCoinAmount = strAnonymizeOuiCoinAmount.remove(strAnonymizeOuiCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOuiCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeOuiCoinAmount*COIN) nMaxToAnonymize = nAnonymizeOuiCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeOuiCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeOuiCoinAmount));
        strAnonymizeOuiCoinAmount = strAnonymizeOuiCoinAmount.remove(strAnonymizeOuiCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOuiCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {

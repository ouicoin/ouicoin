...
int nAnonymizeOuiCoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OUI
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OUI
    // Mac: ~/Library/Application Support/OUI
    // Unix: ~/.OUI
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OUI";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "OuiCoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "OuiCoin.conf";
               ...
               fprintf(ConfFile, "addnode=128.199.244.209\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "OuiCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong OuiCoin will not work properly.");

    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat OuiCoin and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002 OUI per transaction on average.
            */
            if(r <= 10)
            {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeOuiCoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
bool CDarksendPool::SendRandomPaymentToSelf()
{
    ...
    int32_t nChangePos;
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, strTxComment, nChangePos, strFail, coinControl, ONLY_DENOMINATED);
    if(!success){
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, strTxComment, nChangePos, strFail, coinControl, ONLY_NONDENOMINATED_NOT10000IFMN);
    if(!success){
        ...
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                nFeeRet, strTxComment, nChangePos, strFail, coinControl, ONLY_NOT10000IFMN);
        if(!success){
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100 OUI +1 ( bit on if present )
    // bit 1 - 10 OUI +1
    // bit 2 - 1 OUI +1
    // bit 3 - .1 OUI +1
    // bit 3 - non-denom
